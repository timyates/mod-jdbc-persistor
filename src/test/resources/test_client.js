...
function testCreate() {
  eb.send( 'test.persistor', {
    action: 'execute',
    stmt:   'CREATE TABLE testing ( id INTEGER GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1) NOT NULL, name VARCHAR(80), CONSTRAINT testid PRIMARY KEY ( id ) )'
  }, function( reply ) {
    tu.azzert( reply.status === 'ok' ) ;
    tu.azzert( reply.result == undefined ) ;
    tu.testComplete() ;
